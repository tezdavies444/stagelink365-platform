...
- **The calendar sync runs on a Vercel cron.** `vercel.json` declares a once-daily cron hitting `/api/calendar-sync` and a `maxDuration` for that function. Vercel authenticates cron calls by sending `Authorization: Bearer <CRON_SECRET>`; the handler rejects any call without it. The sync is read-only on the two TAD bases and writes only the StageLink `Availability` table — see Open Item #2 and `strategy/calendar-sync/`.
