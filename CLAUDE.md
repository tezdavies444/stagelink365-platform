...
- **Env vars live in Vercel Project → Settings → Environment Variables**, Production scope (and Preview scope if preview deploys are used). Required: `AIRTABLE_API_TOKEN`, `AIRTABLE_BASE_ID`, `ADMIN_TOKEN`, `AIRTABLE_FOUNDERS_TABLE_ID` (= `tblwTL67NIFA2pmQG`, added 2026-04-25 for Open Item #11), and — for the calendar sync (`api/calendar-sync.js`, added 2026-05-21 for Open Item #2) — `TAD_BOOKINGS_BASE_ID`, `CRUISE_ENGAGEMENTS_BASE_ID`, `CLIENT_PORTAL_BASE_ID` (the CLIENT PORTAL base feeding the Cast Cascade connector, added 2026-05-30 for PR #20 — inert until set), and `CRON_SECRET`. Optional: `AIRTABLE_PROFILES_TABLE_ID` (fallback `tblse7dXJfUjvEWQa` is hardcoded in every handler); `TAD_AIRTABLE_API_TOKEN` (a read-only PAT scoped to the two TAD bases — falls back to `AIRTABLE_API_TOKEN`, which must then itself have read access to TAD BOOKINGS and CRUISE ENGAGEMENTS); `GOOGLE_SERVICE_ACCOUNT_KEY` (full service-account JSON for the Performer Google Calendar connector in `api/calendar-sync.js`, added 2026-05-29 — the connector reads each performer's Google calendar read-only via this account and is inert until the var is set; identity is the `Google Calendar ID` Profiles field). For booking notifications (`api/bookings.js`, added 2026-05-30 for the booking-origination work / PR #26): `RESEND_API_KEY` (Resend API key — the email layer is inert without it; a new booking request emails the TAD inbox and a response emails the booker, both via Resend's REST API over `fetch`, no SDK), `RESEND_FROM` (verified-domain sender, e.g. `StageLink365 <bookings@stagelink365.com>` — the domain must be verified in Resend), and `BOOKING_NOTIFY_EMAIL` (the TAD inbox that receives new-request alerts). All three are optional: the in-app "My booking requests" view works without them, and a failed/unconfigured send never breaks the booking write.
...
- **The calendar sync runs on a Vercel cron.** `vercel.json` declares a cron (every 6 hours) hitting `/api/calendar-sync` and a `maxDuration` for that function. Vercel authenticates cron calls by sending `Authorization: Bearer <CRON_SECRET>`; the handler rejects any call without it. The sync is read-only on the three TAD bases and writes only the StageLink `Availability` table — see Open Item #2 and `strategy/calendar-sync/`.
