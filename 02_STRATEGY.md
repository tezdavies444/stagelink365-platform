...
(Note on segments: cruise remains a major *internal* go-to-market lever for SL365 — TAD Shows' cruise rolodex is the asymmetric advantage that powers the 90-day plan in §5 — but it is intentionally absent from **all public-facing copy** across `personnel/index.html` and `index.html`: the homepage H1, sub-tagline, positioning triangle, Founder Tier section, body-copy feature lists, testimonials, venue-type taxonomies, sample gigs, and dropdown/picker options. SL365's outward identity is a standalone casino / corporate / venue-residency marketplace, never named to the buyer as a TAD/cruise extension. The cruise relationships are the operational lever, not the brand.)
...
*Linkage update (v1.4, 2026-05-13).* The 2026-04-28 "periodic manual sweep" linking Profiles to Founders is **partially superseded** by Open Item #17a for the cold-claim path. Talent claims via `api/founder.js` now auto-create a Profile and link `Founders.Profile` at claim time, with `status='converted'` from the start. **Path A — migration claims** (a Profile already exists, then the talent claims a seat) is **not** covered by this change and still needs the manual sweep until a dedicated Path A item ships. Agency/venue claims also skip the auto-create — those are entities, not people, and remain Founders-row only.
...
**7. Homepage tagline — H1 and sub-tagline both locked.** H1: "Serious bookings, not bidding wars." Sub-tagline: "The booking platform built for casino, corporate, and venue residency — where talent, agencies, and venues work together, not against each other." Rationale: H1 is the strongest counter-positioning line in the source doc. Sub-tagline names the chain (talent / agencies / venues) and the segments (casino / corporate / venue residency). **Cruise is intentionally omitted from all public-facing copy** across `personnel/index.html` and `index.html` — homepage H1, sub-tagline, positioning triangle, Founder Tier section, body-copy feature lists, testimonials, venue-type taxonomies, sample gigs, and dropdown/picker options. SL365's outward identity is a standalone casino / corporate / venue-residency marketplace, never named to the buyer as a TAD/cruise extension. Cruise remains a major internal go-to-market lever (TAD's cruise rolodex is the engine of the 90-day plan), but the buyer never sees the word. Do not A/B alternative H1s at launch; one focused message in Month 1, A/B in Months 4–6 once there's traffic to learn from.
...
- *v1.4 (2026-05-13) — Open Item #17a shipped (talent Founder claim → auto-created Profile + magic-link onramp). Added a "Linkage update" paragraph to §7 #5 noting that the 2026-04-28 manual-sweep workflow is now partially superseded for the cold-claim path: talent claims via `api/founder.js` auto-create a Profile, link `Founders.Profile`, and set `status='converted'` from the start. Path A (existing-Profile-claims-seat) still needs the manual sweep until its dedicated OI ships. Agency/venue claims unchanged. No change to caps, perks, badges, or any other §7 decision.*
- *v1.3 (2026-04-26) — Tightened the cruise-omission commitment from v1.2's "four enforced identity surfaces" framing to **all public-facing copy** across `personnel/index.html` and `index.html`. Reverses v1.2's permissive body-copy stance: cruise is now also removed from feature-card text, testimonials, the "Industry Pros" who-card, the venue-type taxonomy and chip list, the AREA_OPTIONS / GIG_TYPES / VENUE_TYPES dropdown arrays, and the sample gig in the venue dashboard. Cruise stays in §5 as an internal go-to-market lever but is never surfaced to the buyer. Driven by Terry's review of production after PR #4 landed: "this is not for cruise lines." Paired with a code PR that scrubs all 10 cruise mentions from both files. Open Item #13's previous resolution (Option 2: narrow the doc) is superseded by this v1.3 (effectively Option 1: scrub the copy).*
